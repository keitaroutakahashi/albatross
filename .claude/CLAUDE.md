...
2. **作業計画**:
   2-1: ステアリングファイル(`.steering/`)での実装を指示された場合は、ステアリングファイルで「今回何をするか」を計画する
   2-2: 直接実装内容を指示された場合は、その指示に従い計画する
3. **実装**:
   3.1: (2-1 の場合) tasklist.md に従って実装し、進捗を随時更新
   3.2: (2-2 の場合) 指示に従って実装し、進捗を随時更新
4. **検証**: テストと動作確認
5. **更新**: 必要に応じてドキュメント更新
